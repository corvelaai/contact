#!/usr/bin/env python3
"""
ContactUpdate v7.5 — consolidated one-file implementation.

Preserves the repository's existing workflow and integrations:
    TikTok profile (ScrapeCreators)
        -> TikTok recent videos (ScrapeCreators)
        -> contact extraction
        -> cross-platform enumeration (Sherlock / Maigret / Tookie)
        -> direct public profile discovery
        -> Twitter/X (ScrapeCreators)
        -> YouTube (YouTube Data API)
        -> Instagram public profile
        -> Telegram public profile / configured Telegram bot cascade
        -> aggregate extraction
        -> Gemini -> Groq AI analysis
        -> evidence + confidence report

The public extraction methods are intentionally based on the repository's
existing regexes and normalization logic. This version improves orchestration,
deduplication, retries, structured output, evidence tracking, and API handling.

It does not bypass platform privacy controls or attempt to obtain contact
information that is not exposed to the configured account/source.
...
Environment used by the original repository:
    SCRAPECREATORS_API_KEY
    GEMINI_API_KEY
    GROQ_API_KEY
    YOUTUBE_API_KEY

Optional enumeration/tool settings:
    SHERLOCK_PATH / SHERLOCK_ENABLED=true
    MAIGRET_PATH / MAIGRET_ENABLED=true
    TOKIE_PATH / TOKIE_ENABLED=true
    TELEGRAM_QUERY_BOTS
    TELEGRAM_BOT_MESSAGE_TEMPLATES
    TELEGRAM_USERBOT_SESSION / TELEGRAM_USERBOT_API_ID /
        TELEGRAM_USERBOT_API_HASH / TELEGRAM_USERBOT_PHONE

Run:
    python3 contactupdate_v7_5_exact.py @username
    python3 contactupdate_v7_5_exact.py @username --json result.json
"""
